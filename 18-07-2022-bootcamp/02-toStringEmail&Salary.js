// Create a function that is receiving a collection of employees and using toString() returns a string with comma-separated    
...
const employees = [
  ...
  // version 1
  function getEmailSallary (employees) {
      let salaryEmail = [];
      for (let employeeData of employees) {
          salaryEmail.push(employeeData.email + ' ' + employeeData.salary);
      }
      return salaryEmail.toString();
  }

  // version 2
  function getEmailSallary (employees) {
    let salaryEmail = employees.map(function (employeeData) {
        return `${employeeData.email}: ${employeeData.salary}`;
    }).toString();
