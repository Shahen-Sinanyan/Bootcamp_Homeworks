...
function findEmployeeByName (employees, name) {
   let employeeData =  employees.find(item => item.firstName === name || item.lastName === name)
   return  employeeData === undefined? `there is not exist ${name}`: employeeData;
};
findEmployeeByName (employees, 'Hendo')
'there is not exist Hendo'
//Create a function that will find the employee by the provided name 
...
findEmployeeByName (employees, 'Gillcrist');